...
function copyText(id) {
  ...
  alert(
    `The text “${document.getElementById(id).innerText}” has been copied to your Clipboard.`,
  );
}
...
function showMenu(id) {
  if (!dMenuLocked.includes(1)) {
    ...
    document.getElementById(idButton).style.backgroundColor = getComputedStyle(
      document.querySelector(":root"),
    ).getPropertyValue("--hover-colour");
...
function lockMenu(id) {
  ...
  if (menuLockStatus == 0) {
    ...
    document.getElementById(idButton).style.backgroundColor = getComputedStyle(
      document.querySelector(":root"),
    ).getPropertyValue("--browser-theme-colour"); // to indicate that dropdown is locked
    document.getElementById(id).style.height = dMenuHeights[dMenus.indexOf(id)];
  }
}
...
document.addEventListener("click", function (event) {
  if (
    !(
      dMenus.includes("d" + event.target.id.slice(1)) ||
      dMenus.includes(event.target.id)
    )
  ) {
    dMenuLocked = [0, 0, 0, 0, 0, 0];
    hideAllMenus();
